  ...
  return (
    <div className="mb-5">
      ...
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-medium text-ink-2">At a glance · {monthStr}</p>
        <button
          onClick={() => setOpen(v => !v)}
          className="text-xs text-accent bg-transparent border-none cursor-pointer p-0 hover:underline"
        >
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="card kpi">
          ...
          {kpiLoad ? <Skeleton /> : <p className="kpi-value">{R(kpi.cash)}</p>}
          <p className="kpi-sub">{primaryBank ? primaryBank.name : 'No bank linked'}</p>
        </div>

        <div className="card kpi">
          <p className="kpi-label">Customers</p>
          {kpiLoad ? <Skeleton /> : <p className="kpi-value">{R(kpi.ar)}</p>}
        ...
        <div className="card kpi">
          ...
          {chLoad ? <Skeleton /> : <p className="kpi-value" style={{ color: profitColor }}>{R(monthProfit)}</p>}
          <p className="kpi-sub">
        ...
        <div className="card-accent kpi">
          ...
          {kpiLoad ? <Skeleton /> : <p className="kpi-value" style={{ color: 'var(--accent)' }}>{R(kpi.vat)}</p>}
          <p className="kpi-sub" style={{ color: 'var(--accent)' }}>due {vatDue}</p>
        </div>
      </div>
